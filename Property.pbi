...
Procedure Property_Window( Flag = #PB_Window_ScreenCentered, ParentID = 0, Width = 231, Height = 276 )
  ...
  If IsWindow( Window )
    ...
    Else
      If ((Flag & #PB_Window_NoActivate) = #PB_Window_NoActivate)
        CloseWindow( Window ) :Property_Window( Flag, ParentID, Width, Height )
      Else
...
Runtime Procedure Property_Window_Show( ParentID = #False, Flag = #PB_Window_ScreenCentered )
  If Not Flag
    If ParentID 
    ...
    EndIf
  EndIf
  Protected Window = Property_Window( Flag, ParentID )
