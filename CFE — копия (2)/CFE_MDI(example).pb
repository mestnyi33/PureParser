; 05,04,2017 глобальные изменения
...
Procedure DrawMDIElementItemsContent(List This.S_CREATE_ELEMENT())
...
Procedure.q SetMDIElementFlag(Flag.q)
...
Procedure.q GetMDIElementFlag(Flag.q)
  Protected Result.q = Flag
...
;- PRIVATE
Procedure MDIBarEvent(Event.q, EventElement)
...
Procedure ResizeMDIScrollVert(List This.S_CREATE_ELEMENT(), Height, bSize)
...
Procedure ResizeMDIScrollHorz(List This.S_CREATE_ELEMENT(), Width, bSize)
...
Procedure MDISizeEvent(Event.q, EventElement)
...
Procedure _MDISizeEvent(Event.q, EventElement)
...
;- PUBLIC
Procedure GetMDIElementAttribute(List This.S_CREATE_ELEMENT(), Attribute)
  Protected Result
...
Procedure SetMDIElementAttribute(List This.S_CREATE_ELEMENT(), Attribute, Value)
...
Procedure MDIElement( Element, X,Y,Width,Height, ScrollAreaWidth,ScrollAreaHeight, ScrollStep, Flag.q = #_Flag_Double, Parent =- 1 )
      ...
      Case 10
       ...
       Select Event
          ...
          Case #_Event_MouseMove
            If GetButtons()
              With *CreateElement
                Protected Value = GetMouseY()-(ElementY(EventElement, #_Element_FrameCoordinate)-8-4)
                If Value > GetElementAttribute(10, #PB_ScrollArea_InnerHeight)
                  ChangeCurrentElement(\This(), ElementID(10))
                  SetMDIElementAttribute(\This(), #PB_ScrollArea_InnerHeight, Value)
                  ;SetElementAttribute(10, #PB_ScrollArea_InnerHeight, GetMouseY()-(ElementY(EventElement, #_Element_FrameCoordinate)-8-4))
                EndIf
              EndWith
            EndIf
