...
Procedure CO_Insert(Parent)
  ...
  With ParsePBObject()
    ...
    *This\get(\Object\Argument$)\Code("Code_Function")\Position = \Content\Position+\Content\Length+Len(#CRLF$+Space(Indent))
    
    ; 
    If ParsePBObject()\Container
      Select ParsePBObject()\Container
        Case #PB_GadgetType_Panel, #PB_GadgetType_Container, #PB_GadgetType_ScrollArea, #PB_GadgetType_Canvas
          *This\Content\Text$ = InsertString(*This\Content\Text$, "CloseGadgetList()"+#CRLF$+Space(Indent), *This\get(\Object\Argument$)\Code("Code_Function")\Position) 
...
Procedure CO_Create(Type$, X, Y, Parent=-1)
  Protected GadgetList, Constant.s, Object, Position
  ...
  With *This
    ...
    *Parse = AddElement(ParsePBObject())
    If *Parse
      ; Использовать ли #PB_Any?
      If Bool(Asc(GetGadgetText(Properties_ID))='#' Or state_id=0)
        Constant = "#"
      Else
        Constant = ""
      EndIf
      ...
      If state_class
        ...
        Else
          \Object\Argument$ = Constant+\Class\Argument$
        EndIf
      Else
