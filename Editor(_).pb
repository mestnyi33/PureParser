...
Procedure wgParent(Object.i) ; Ok
  If IsGadget(Object)
    Protected *wg.WindowStruct = GetGadgetData(Object)
    If *wg And GadgetType(Object) = #PB_GadgetType_Canvas  
      ProcedureReturn *wg\Form
    Else
      ProcedureReturn Object
    EndIf
  EndIf
...
Procedure.i WindowMatch(Value.i, Grid.i, Max.i=$7FFFFFFF)
  Value = Round((Value/Grid), #PB_Round_Nearest) * Grid
  If (Value>Max) : Value=Max : EndIf
  ProcedureReturn Value
EndProcedure

Procedure WindowContentDraw(Gadget, Steps)
  Protected x,y,w,h, BackColor
...
Procedure WindowUpdate(*wg.WindowStruct)
  ...
  With *wg
    ...
    If StartDrawing(CanvasOutput(\Form))
      If w > OutputWidth() : w = OutputWidth() : EndIf
      If h > OutputHeight() : h = OutputHeight() : EndIf
...
Procedure FormCallBack()
  ...
  With *wg
    Select EventType()
      Case #PB_EventType_Size;, #PB_EventType_Resize
        \i\x = WindowMatch(\BorderSize, \Steps)
        \i\y = WindowMatch(\CaptionSize+\BorderSize, \Steps)
        \i\width = w-\BorderSize*2
        \i\height = h-\i\y-\BorderSize
        SetGadgetData(\Form, *wg)
        
        WindowUpdate(*wg)
        ResizeGadget(\Gadget, \i\x, \i\y, \i\width, \i\height)
        WindowContentDraw(\Gadget, \Steps-1)
...
Procedure WindowCallBack()
  ...
  Protected *wg.WindowStruct = GetGadgetData(Gadget)
  PostEvent(#PB_Event_Gadget, EventWindow(), *wg\Form, EventType(), Gadget)
EndProcedure

Procedure WindowGadget(Gadget, X,Y,Width,Height, Title$="", Flag=0, Parent=-1)
  ...
  With *wg
    ...
    \Form = CanvasGadget(#PB_Any, x-\BorderSize,y-\CaptionSize-\BorderSize, Width+\BorderSize*2,Height+\CaptionSize+\BorderSize*2, #PB_Canvas_Container|#WS_CLIPSIBLINGS) 
    \Gadget = CanvasGadget(Gadget, \BorderSize,\CaptionSize+\BorderSize,Width,Height, #PB_Canvas_Container) : CloseGadgetList() : If IsGadget(Gadget) : \Gadget = Gadget : EndIf
    CloseGadgetList()
    ...
    BindGadgetEvent(\Form, @FormCallBack())
    BindGadgetEvent(\Gadget, @WindowCallBack(), #PB_EventType_LeftButtonDown)
